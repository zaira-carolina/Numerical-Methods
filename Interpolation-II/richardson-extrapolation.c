...
void extraRichardson(int cantIntervalos, int n, double a, double b, double hvalue){
	...
	for (i=0; i<aprox; i++){
		...
		for (j=1; j<aprox; j++){
			if (i>=j){
				Laprox[i][j] = (pow(2, j)*Laprox[i][j-1]-Laprox[i-1][j-1])/(pow(2, j)-1); 
			}
		}
	}
...
int main(){
	clock_t start = clock(); 
	int cantIntervalos = 2; // NO CAMBIAR, ESTE CAMBIA CON LA EXTRAPOLACION DE RICHARDSON
	int n = 1; // ESTE PUEDE CAMBIAR
	double a = 0.0, b = PI; // LIMITES DE LA INTEGRAL
