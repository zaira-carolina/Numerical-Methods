...
double *vector_resta(int N, double *v1, double *v2){
	...
	double *x_grad(double **A, double *b, int filas, int cols, double TOL){
		double *xk, *x0; 
		double *r, alpha; 
		int k = 0;
		...
		while (error >  TOL){
			...
			alpha = vector_vector(r, r, filas)/(vector_vector(r, matriz_vector(A, r, filas), filas));
...
int main(){
	...
	//Solucion
	double *x; 
	x = x_grad(A, b, filas, cols, 0.0001); 
	//for (int i=0; i<filas; i++){
	//	printf("\n %f", x[i]); 
	//	}
		
	save_vector(x, filas, "x_grad_3x3.txt"); 
